...
import { fetchRestaurants, deleteRestaurant } from '../services/admin.service';
import AddRestaurantModal from '../components/eatdrink/AddRestaurantModal';
import EditRestaurantModal from '../components/eatdrink/EditRestaurantModal';
  ...
  return (
    <div className="bg-white rounded-[20px] flex items-center px-4 py-0 h-[90px] gap-4">
      ...
      <div className="w-[60px] h-[60px] rounded-[10px] overflow-hidden flex-shrink-0 bg-gray-100">
        {item.coverPhoto ? (
          <img src={item.coverPhoto} alt={item.nameEn} className="w-full h-full object-cover" />
        ) : (
      ...
      <div className="w-[220px] shrink-0">
        ...
        <p className="text-[15px] text-[#4e637f] truncate">{item.nameEn}</p>
      </div>

      {/* City Destination */}
      <div className="w-[160px] shrink-0">
        <p className="text-[16px] font-medium text-[#232323]">City</p>
        <p className="text-[15px] text-[#4e637f] truncate">{item.destination?.city || '—'}</p>
      </div>

      {/* Location */}
      <div className="w-[160px] shrink-0">
        <p className="text-[16px] font-medium text-[#232323]">Address</p>
        <p className="text-[15px] text-[#4e637f] truncate">{item.location || '—'}</p>
      </div>

      {/* Description / Type */}
      <div className="flex-1 min-w-0">
        <p className="text-[16px] font-medium text-[#232323]">Description</p>
        <p className="text-[15px] text-[#4e637f] truncate">{item.description || '—'}</p>
      </div>
...
export default function EatDrink() {
  ...
  const load = useCallback(() => {
    ...
    fetchRestaurants(params)
      .then(({ restaurants: rows, pagination: pg }) => {
        setItems(rows);
        setPagination(pg);
      })
      .catch(console.error)
  ...
  const handleDelete = async () => {
    ...
    try { await deleteRestaurant(deleteId); load(); }
    catch { alert('Failed to delete restaurant. Please try again.'); }
  ...
  return (
    <div className="space-y-5">
      {/* Header */}
      <h1 className="text-[32px] font-bold text-[#202224] tracking-[-0.1px]">Eat &amp; Drink</h1>
      ...
      <div className="relative w-[388px]">
        ...
        <input
          type="text"
          placeholder="Search for Eat & Drink"
          value={search}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 gap-2">
          <p className="text-sm font-medium text-gray-600">No restaurants found</p>
          <p className="text-xs text-gray-400">Add restaurants to see them here</p>
        </div>
      ) : (
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-1">Delete Restaurant</h3>
            <p className="text-sm text-gray-500 mb-6">This restaurant will be deactivated and hidden from the app.</p>
