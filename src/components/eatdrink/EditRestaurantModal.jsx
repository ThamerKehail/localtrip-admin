...
import { updateRestaurant, uploadImage, fetchDestinations } from '../../services/admin.service';

export default function EditRestaurantModal({ isOpen, onClose, onSave, restaurant }) {
  const [form, setForm] = useState({ nameEn: '', nameAr: '', description: '', location: '', tips: '', destinationId: '', imageFile: null, imagePreview: null });
  ...
  useEffect(() => {
    if (restaurant) {
      setForm({
        ...
        imageFile: null,
        imagePreview: restaurant.coverPhoto || null,
      });
      setErrors({});
    }
  }, [restaurant]);
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      let coverPhoto = form.imageFile ? await uploadImage(form.imageFile, 'localtrip/restaurants') : (form.imagePreview || null);
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Edit Restaurant" size="lg">
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        {/* Destination (City) */}
        <div>
          <label className="text-sm font-medium text-gray-700 block mb-1.5">City Destination</label>
          <select
